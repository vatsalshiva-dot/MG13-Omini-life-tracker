...
export const DailyTrackerView: React.FC<DailyTrackerViewProps> = ({
  ...
  // Voice Auto-Log States
  const [isVoiceLogActive, setIsVoiceLogActive] = useState(false);
  ...
  const toggleVoiceRecording = async () => {
    ...
    try {
        ...
        alert("🎤 Microphone Access Blocked:\n\nPlease grant permissions or open this app in a new tab to bypass iframe sandbox limits.");
        return;
    }
    ...
    let localTranscript = accumulatedTranscript ? accumulatedTranscript + ' ' : '';
  ...
  useEffect(() => {
    if (autoStartVoice) {
      setIsVoiceLogActive(true);
      if (!isVoiceRecording) {
         toggleVoiceRecording();
      }
      if (onClearAutoStartVoice) {
         onClearAutoStartVoice();
      }
    }
  }, [autoStartVoice, isVoiceRecording, onClearAutoStartVoice]);

  const handleStopAndParse = async () => {
     // Stop mic and recorder
     if (isVoiceRecording) {
        if (recognitionRef.current) {
          try { recognitionRef.current.stop(); } catch(e) {}
        }
     ...
     if (!finalTranscript) {
        alert("Please speak or write something first!");
        return;
     }
     ...
     try {
        ...
        // Wait a brief period to let mediaChunks populate
        ...
        // Save the audio physical file to the Journal entry if recorded
        if (onSaveJournal && b64Audio) {
           const currentJournal = state.journals[date] || {
              date: date,
              mood: 0,
              energy: 0,
              tags: [],
           ...
           onSaveJournal(date, {
              audioLog: b64Audio || (currentJournal as any).audioLog,
           });
        }

        // Now call Omni Assistant Parser API
        ...
        if (!res.ok) throw new Error("Voice mapper service failed");

        const parsedData = await res.json();

        // Direct execution!
        if (onOmniCommand) {
           onOmniCommand(parsedData);
        }

        // Clean up panel
        setAccumulatedTranscript('');
        setIsVoiceLogActive(false);

     } catch (err: any) {
        console.error(err);
        alert(`Failed to parse voice command: ${err.message}`);
     } finally {
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-[#111120] pb-5">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3">
          ...
          <button
            onClick={() => setIsVoiceLogActive(prev => !prev)}
            className={`px-3 py-1.5 text-[10px] font-bold rounded-lg border transition-all duration-155 tracking-wider uppercase flex items-center gap-1.5 cursor-pointer ${
              isVoiceLogActive 
                ? 'bg-rose-500/20 text-rose-500 border-rose-500/40 animate-pulse' 
                : 'bg-[#ff00a0]/10 text-[#ff00a0] border-[#ff00a0]/30 hover:bg-[#ff00a0]/20'
            }`}
          >
            <Mic size={11} className={isVoiceLogActive ? 'animate-pulse' : ''} />
            Voice Auto-Log
          </button>
        </div>
      </div>
      ...
      {isVoiceLogActive && (
        <div className="bg-[#0b0b18] border-2 border-[#ff00a0]/40 rounded-2xl p-5 shadow-[0_0_25px_rgba(255,0,160,0.15)] relative overflow-hidden animate-in fade-in duration-200">
          <div className="absolute top-0 right-0 w-32 h-32 bg-[#ff00a0]/5 blur-3xl pointer-events-none" />
          
          <div className="flex items-center justify-between mb-3 border-b border-[#201030] pb-3">
            <div className="flex items-center gap-2">
              <span className="relative flex h-2 w-2">
                <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${isVoiceRecording ? 'bg-rose-500' : 'bg-[#ff00a0]'}`}></span>
                <span className={`relative inline-flex rounded-full h-2 w-2 ${isVoiceRecording ? 'bg-rose-600' : 'bg-[#ff00a0]'}`}></span>
              </span>
              <h3 className="text-xs font-black uppercase tracking-widest text-[#ff00a0] font-mono">
                🎤 Advanced Voice Auto-Log Engine
              </h3>
            </div>
            <button 
              onClick={() => {
              ...
              className="text-slate-500 hover:text-slate-300 text-xs font-mono font-bold cursor-pointer transition border-b border-transparent hover:border-slate-300 pb-0.5"
            >
              [ ESC CLOSE ]
            </button>
          </div>
          ...
          <p className="text-[10px] text-slate-400 font-mono mb-2 leading-relaxed">
            // This engine uses advanced speech mapping to instantly record routines, log checkboxes, edit baseline goals/reps targets, stream financials, or create reminders. Spoken diary notes are auto-categorized into your daily journal sections!
          </p>

          <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 p-2.5 bg-[#ff00a0]/5 border border-[#ff00a0]/15 rounded-xl text-[9px] font-mono text-slate-400 leading-normal">
             <span className="text-[#ff00a0] font-black shrink-0">🎙️ VOICE JOURNAL TIPS:</span>
             <span>Speak section names directly (e.g., <strong className="text-rose-400">"Wins: had a super productive day"</strong>) or add new custom ones (e.g., <strong className="text-rose-400">"add heading chemotherapy and write blood sugar normal"</strong>). Any unmatching stories write directly into your <strong className="text-[#00d4ff]">Daily Reflection</strong>.</span>
          </div>
          ...
          <div className="space-y-4">
            <textarea
              ...
              placeholder={isVoiceRecording ? "Listening closely... Speak your commands or customize this text..." : "Say or type commands like: 'set study chest workout target to 15 reps', 'marked chem study as done with notes very intense study session', or click the button below to start voice transcription..."}
              className={`w-full min-h-[110px] bg-[#04040a] border rounded-xl p-4 text-xs text-slate-200 focus:outline-none transition-all ${isVoiceRecording ? 'border-[#ff00a0]/60 shadow-[3px_3px_15px_rgba(255,0,160,0.08)]' : 'border-[#1e1e38] focus:border-[#ff00a0]/50'} font-mono leading-relaxed placeholder-slate-600`}
            />
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button
                ...
                className={`flex-1 flex justify-center items-center gap-2 px-5 py-3 rounded-xl text-[10px] font-black uppercase tracking-wider cursor-pointer transition-all ${
                  isVoiceRecording 
                    ? 'bg-rose-500/20 text-rose-500 border border-rose-500/40 animate-pulse shadow-[0_0_15px_rgba(244,63,94,0.1)]' 
                    : 'bg-[#ff00a0]/10 text-[#ff00a0] border border-[#ff00a0]/30 hover:bg-[#ff00a0]/20'
                }`}
              >
                {isProcessingVoice ? <Loader size={13} className="animate-spin" /> : isVoiceRecording ? <MicOff size={13} /> : <Mic size={13} />}
                {isProcessingVoice ? 'PROCESSING...' : isVoiceRecording ? 'STOP TRANSCRIBING' : 'START MIC TRANSCRIPTION'}
              </button>

              <button
                onClick={handleStopAndParse}
                disabled={isProcessingVoice || !accumulatedTranscript.trim()}
                className="flex-1 bg-[#00d4ff]/10 hover:bg-[#00d4ff]/20 text-[#00d4ff] border border-[#00d4ff]/30 hover:border-[#00d4ff]/60 px-5 py-3 rounded-xl text-[10px] font-black tracking-widest uppercase transition disabled:opacity-40 flex items-center justify-center gap-2 cursor-pointer shadow-[0_0_12px_rgba(0,212,255,0.05)] hover:shadow-[0_0_20px_rgba(0,212,255,0.2)]"
              >
                {isProcessingVoice ? <Loader size={13} className="animate-spin" /> : <Sparkles size={13} />}
                {isProcessingVoice ? 'PLANNING ACTIONS...' : 'STOP & PARSE ACTIONS'}
              </button>
            </div>
          </div>
        </div>
