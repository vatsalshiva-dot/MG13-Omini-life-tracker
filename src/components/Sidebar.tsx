...
import { 
  Home, Clipboard, Target, BarChart2, Calendar, Bell, 
  Clock, Book, Share2, Search, Settings, Cloud, Download, Bot, HelpCircle,
  AlertCircle, Headphones, X, Wind, Droplets, CloudLightning, Waves, Coffee, Music, Volume2
} from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const userAlertsCount = React.useMemo(() => {
    ...
    return state.reminders.filter(r => r.status !== 'done' && r.dueDate <= today).length;
  }, [state.reminders]);

  const systemAlertsCount = hasSystemAlerts ? 1 : 0;

  const sections = [
    { id: 'dashboard', label: 'Dashboard', icon: <Home size={14} />, section: 'OVERVIEW' },
    { id: 'daily', label: 'Daily Tracker', icon: <Clipboard size={14} />, section: 'DAILY LOG' },
    { id: 'journal', label: 'Daily Journal', icon: <Book size={14} />, section: 'DAILY LOG' },
    { id: 'goals', label: 'Goals & Targets', icon: <Target size={14} />, section: 'GOALS & ANALYSIS' },
    { id: 'analytics', label: 'Analytics', icon: <BarChart2 size={14} />, section: 'GOALS & ANALYSIS' },
    { id: 'calendar', label: 'Calendar View', icon: <Calendar size={14} />, section: 'GOALS & ANALYSIS' },
    { id: 'ai_analyst', label: 'AI Analyst Hub', icon: <Bot size={14} />, section: 'EXTENSIONS' },
    { id: 'focus_audio', label: 'Focus Audio', icon: <Headphones size={14} />, section: 'EXTENSIONS' },
    { id: 'expeditions', label: 'Expeditions', icon: <Search size={14} />, section: 'EXTENSIONS' },
    { id: 'finances', label: 'Finances', icon: <Target size={14} />, section: 'EXTENSIONS' },
    { id: 'sketchpad', label: 'Sketchpad', icon: <Share2 size={14} />, section: 'EXTENSIONS' },
    { id: 'reminders', label: 'Reminders & Alerts', icon: <Bell size={14} />, badgeCount: userAlertsCount, section: 'MANAGE' },
    { id: 'pomo', label: 'Pomodoro Clock', icon: <Clock size={14} />, section: 'MANAGE' },
    { id: 'synopsis', label: 'Synopsis & Share', icon: <Share2 size={14} />, section: 'MANAGE' },
    { id: 'search', label: 'Search', icon: <Search size={14} />, section: 'MANAGE' },
    { id: 'settings', label: 'Settings', icon: <Settings size={14} />, section: 'MANAGE' },
    { id: 'alerts', label: 'Developer Alerts', icon: <AlertCircle size={14} />, badgeCount: systemAlertsCount, section: 'MANAGE' },
    { id: 'help', label: 'Help & Info', icon: <HelpCircle size={14} />, section: 'MANAGE' },
    { id: 'demo', label: 'Load Live Demo', icon: <CloudLightning size={14} />, section: 'MANAGE' },
  ];
    ...
    <nav className="w-[210px] min-w-[210px] bg-[#0d0d1a] border-r border-[#111120] flex flex-col h-screen sticky top-0 overflow-y-auto font-semibold scrollbar-none select-none">
      ...
      >
        <h1 className="text-sm font-extrabold tracking-wider text-white">
          OMNILIFE <span id="brand-accent" className="text-[#ff6b1a]">TRACKER</span>
        </h1>
            ...
            <div className="space-y-0.5">
              {list.map(s => {
                ...
                return (
                  ...
                  >
                    <span className={`shrink-0 ${isActive ? 'text-[#ff6b1a]' : 'text-slate-500'}`}>
                      {s.icon}
                    </span>
