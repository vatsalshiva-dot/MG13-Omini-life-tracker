...
import { OffGridSyncPanel } from './OffGridSyncPanel';
import { 
...
export const SettingsView: React.FC<SettingsViewProps> = ({
  ...
  // OffGrid states structured autonomously inside the child panel

  const handleLinkGhost = async () => {
    const isSafariMobile = !('showOpenFilePicker' in window);
    if (isSafariMobile) {
      alert("Mobile & Safari detected! Activating high-integrity Decentralized Sandboxed Local Storage Node (OPFS). Let's pick your JSON file to initialize!");
    }
    const ok = await linkGhostSyncFile();
    setGhostLinked(ok);
    if (ok) {
      alert(isSafariMobile 
        ? '🎉 Ghost Sync Sandboxed Node Linked! All future edits will auto-save transparently on this device.'
        : '🎉 Ghost Sync File Linked! Data will auto-save transparently on next edits.');
    }
  };
  
  const handleCreateGhost = async () => {
    const isSafariMobile = !('showSaveFilePicker' in window);
    if (isSafariMobile) {
      alert("Mobile & Safari detected! Activating sandboxed local node. We are initializing your offline workspace and downloading your master backup baseline JSON file immediately!");
    }
    const ok = await createGhostSyncFile();
    setGhostLinked(ok);
    if (ok) {
      alert(isSafariMobile
        ? '🎉 Private Sandboxed Node Created! All edits auto-save instantly. Baseline backup config file triggered for local download!'
        : '🎉 New Ghost Sync File Created. Data will auto-save transparently on next edits.');
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <OffGridSyncPanel state={state} onImportJSON={onImportJSON} />
      ...
      <div className="bg-[#0d0d1a] border border-[#1e1e38] rounded-xl p-5 space-y-4 shadow-[0_0_20px_rgba(255,107,26,0.05)]">
        <h3 className="text-xs font-black uppercase tracking-wider text-[#ff6b1a] border-b border-[#111120] pb-2 flex items-center justify-between gap-1.5">
          <span className="flex items-center gap-1.5">
            ...
            <span className="text-[7.5px] bg-[#00ff88]/10 border border-[#00ff88]/30 text-[#00ff88] px-2 py-0.5 rounded font-black tracking-widest uppercase">⭐ RECOMMENDED DECENTRALIZED</span>
          </span>
          <button 
            type="button" 
